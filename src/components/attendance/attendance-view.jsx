...
function AttendanceView({ classYear, students, teacherSubjects }) {
    ...
    useEffect(() => {
        ...
        const fetchAttendances = async () => {
            const res = await getRequest(`http://192.168.1.9:8000/attendance/${classYear._class.id}/${selectedTeacherSubject}/?date=${date}`)
    ...
    async function onSubmit(attendances) {
        ...
        const res = await postRequest("http://192.168.1.9:8000/attendance/", attendancesData)
        const data = await res.json()
        if (res.ok) {
            toast({
                variant: "success",
