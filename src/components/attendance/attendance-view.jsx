...
function AttendanceView({ classYear, students, teacherSubjects }) {
    ...
    useEffect(() => {
        ...
        const fetchAttendances = async () => {
            const res = await getRequest(`mascate-be.onrender.com/attendance/${classYear._class.id}/${selectedTeacherSubject}/?date=${date}`)
    ...
    async function onSubmit(attendances) {
        ...
        const res = await postRequest("mascate-be.onrender.com/attendance/", attendancesData)
        const data = await res.json()
        if (res.ok) {
            toast({
                variant: "success",
