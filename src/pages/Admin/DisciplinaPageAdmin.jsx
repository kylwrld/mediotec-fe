...
function DisciplinaPageAdmin() {
    ...
    useEffect(() => {
        const fetchSubject = async () => {
            const response = await getRequest(`http://127.0.0.1:8000/subject/${id}/`);
            const data = await response.json();
            setSubject(data);
        };
        const fetchTeacherSubjects = async () => {
            const response = await getRequest(`http://127.0.0.1:8000/subject/${id}/teachers/`);
            const data = await response.json();
        ...
        <div className="h-full">
            ...
            <Tabs defaultValue="teachers" className="py-4">
                ...
                <TabsContent value="teachers">
                    <CustomDataTable table={teachersTable}>
                    <TeacherController
                        table={teachersTable}
                    />
                    </CustomDataTable>
